      ...
      <div className='flex mt-4'>
        ...
        <div className='flex md:hidden gap-4 mt-8 '>
                  {entities.map((event,index)=>{
                    return <div key={event.sector} className={`h-[20px] p-1 w-[20px]  flex justify-center items-center rounded-full ${activeIndex === index? 'bg-[var(--primary-blue)]':"bg-white"}  cursor-pointer`} onClick={()=>handleChange(index)}></div>
                  })}
                </div>
        </div>
